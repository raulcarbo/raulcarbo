...
Usa `search_threads` con `in:inbox is:unread -in:draft` (ajusta el rango con `newer_than:` según lo que pida Raúl; por defecto `newer_than:3d`). Si Raúl pide otro filtro (un remitente, un tema, "hoy"), tradúcelo a sintaxis de Gmail.
...
- **Solo informar**: notificaciones, newsletters, facturas automáticas, spam que pasó el filtro, correos internos que no piden nada. NO crees borrador para estos; solo repórtalos en una línea.
